...
This document describes the technical architecture of the Vacuum Scheduler custom component for Home Assistant.

## Directory Structure

```text
custom_components/vacuum_scheduler/
...
│   ├── __init__.py          # Exports VacuumSchedulerDataUpdateCoordinator
...
│   ├── __init__.py          # Exports VacuumSchedulerEntity
...
- `base.py` - Main coordinator class (`VacuumSchedulerDataUpdateCoordinator`)
...
**Key class:** `VacuumSchedulerDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `VacuumSchedulerApiClient`
...
- `VacuumSchedulerConfigFlowHandler` (main flow)
- `VacuumSchedulerOptionsFlow` (options)
...
**Key class:** `VacuumSchedulerEntity` (in `entity/base.py`)
...
2. `VacuumSchedulerEntity` for common functionality
...
1. Create directory: `custom_components/vacuum_scheduler/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `VacuumSchedulerEntity`
