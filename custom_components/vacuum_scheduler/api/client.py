"""
API Client for vacuum_scheduler.
...
class VacuumSchedulerApiClientError(Exception):
    """Base exception to indicate a general API error."""


class VacuumSchedulerApiClientCommunicationError(
    VacuumSchedulerApiClientError,
):
    """Exception to indicate a communication error with the API."""


class VacuumSchedulerApiClientAuthenticationError(
    VacuumSchedulerApiClientError,
):
...
def _verify_response_or_raise(response: aiohttp.ClientResponse) -> None:
    ...
    Raises:
        VacuumSchedulerApiClientAuthenticationError: For 401/403 errors.
        aiohttp.ClientResponseError: For other HTTP errors.

    """
    if response.status in (401, 403):
        msg = "Invalid credentials"
        raise VacuumSchedulerApiClientAuthenticationError(
            msg,
        )
    response.raise_for_status()


class VacuumSchedulerApiClient:
    """
    ...
    async def async_get_data(self) -> Any:
        ...
        Raises:
            VacuumSchedulerApiClientAuthenticationError: If authentication fails.
            VacuumSchedulerApiClientCommunicationError: If communication fails.
            VacuumSchedulerApiClientError: For other API errors.
    ...
    async def _api_wrapper(
        ...
        try:
            ...
            raise VacuumSchedulerApiClientCommunicationError(
                msg,
            ) from exception
            ...
            raise VacuumSchedulerApiClientError(
                msg,
            ) from exception
