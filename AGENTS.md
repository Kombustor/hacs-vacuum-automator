...
- **Repository:** Kombustor/hacs-vacuum-automator

**Key directories:**

- `custom_components/vacuum_scheduler/` - Main integration code
...
- Integration logs: `custom_components.vacuum_scheduler: debug` in `config/configuration.yaml`
...
- **Domain:** `vacuum_scheduler`
- **Title:** Vacuum Scheduler
- **Class prefix:** `VacuumScheduler`

**When creating new files:**

- Use the domain `vacuum_scheduler` for all DOMAIN references
- Prefix all integration-specific classes with `VacuumScheduler`
- Use "Vacuum Scheduler" as the display title
...
- Inherit from platform base + `VacuumSchedulerEntity`
...
- `tests/` mirrors `custom_components/vacuum_scheduler/` structure
